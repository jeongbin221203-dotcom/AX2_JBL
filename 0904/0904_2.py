...
name = st.text_input("1) 이름을 입력하세요", value="홍길동", key="widget_name") # key > 변수
# age = st.text_input("2) 나이을 입력하세요", value="홍길동", key="widget_age")

# 2) 슬라이드 위젯 : 최소/최대/기본값 지정해 숫자로 선택하게 한다.
age = st.slider("2) 나이를 선택하세요", min_value=10, max_value=80, value=25, key="widget_age") # key > 변수
...
# 7) 슬라이더 만족도 점수 (1~5)
point = st.slider("7) 이 강의 만족도 점수 (1~5점)", min_value=1, max_value=5, value=3, key="widget_point")
# 1, 5, 3 > 표기 가능 (min.max,value)
...
st.subheader("☎️실시간 응답 요약")

# 위젯 값들을 버튼을 누르지 않아도 조작하는 즉시 바로 갱신된다.
# st.write(f"- 이름 : **{name}** / 나이 : **{age}**")
# st.write(f"- 직군 : **{job}** / 관심 국가 : **{country}**")
# st.write(f"- 관심분야 : **{interests}**")
# st.write(f"- 강의 만족 여부 : **{agree}**")
# st.write(f"- 강의 만족 점수 : **{point}**")
...
#     st.write(f'- 강의 만족 여부 : {"만족" if agree else "미체크"} / 만족도 점수: **{point}점** ')
...
if submitted:
    # f-string 중첩 에러 방지를 위해 변수로 먼저 가공
    interests_str = ", ".join(interests) if interests else "선택없음"
    agree_str = "만족" if agree else "미체크"
    feedback_str = feedback if feedback else "(작성안함)"

    st.write(f"- 이름 : **{name}** / 나이 : **{age}**")
    st.write(f"- 직군 : **{job}** / 관심 국가 : **{country}**")
    st.write(f"- 관심분야 : **{interests_str}**")
    st.write(f"- 강의 만족 여부 : {agree_str} / 만족도 점수: **{point}점**")
    st.write(f"- 자유 의견 : {feedback_str}")
